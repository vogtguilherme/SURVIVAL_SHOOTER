...
public class EnemyMovement : MonoBehaviour
{
    ...
    public Vector3 velocity { get => m_NavMeshAgent.velocity; }
    ...
    IEnumerator PatrolRoute(Vector3[] waypoints)
    {
        ...
        while (true)
        {
            ...
            {
                if (m_NavMeshAgent.isStopped == true) m_NavMeshAgent.isStopped = false;

                if (m_Enemy.PlayerOnSight)
                {
                    m_NavMeshAgent.SetDestination(m_Enemy.TargetTranform.position);
                }
                else
                {
